...
use devforge_rpc::plugin::VoltMetadata;
...
#[test]
fn test_load_volt() {
    let devforge_proxy_dir = std::env::current_dir()
    ...
    let path = devforge_proxy_dir.join("some-path");
    ...
    if let Ok(volt_metadata) = load_volt(&devforge_proxy_dir) {
        panic!(
            "Unexpected result from `devforge_proxy::plugin::wasi::load_volt` function: {volt_metadata:?}"
        );
    }

    // Invalid file (not readable into a string)
    // Making sure the file exists
    let path = devforge_proxy_dir.join("smiley.png");
    ...
    let parent_path = devforge_proxy_dir.join("some_author.test-plugin-one");
    ...
    let parent_path = devforge_proxy_dir.join("some_author.test-plugin-two");
    ...
    let parent_path = devforge_proxy_dir.join("some_author.test-plugin-three");
