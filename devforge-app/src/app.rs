...
struct Cli {
    ...
    #[clap(value_parser = devforge_proxy::cli::parse_file_line_column)]
...
pub fn launch() {
    ...
    if !cli.wait {
        ...
        if let Err(why) = cmd
            ...
            .stderr(stderr)
            .stdout(stdout)
            .env("LAPCE_LOG", "devforge_app::app=error,off")
            .spawn()
        {
    ...
    if let Err(err) = devforge_proxy::register_lapce_path() {
        tracing::error!("{:?}", err);
    }
...
pub fn try_open_in_existing_process(
    ...
    devforge_rpc::stdio::write_msg(&mut socket, msg)?;
        ...
        std::thread::spawn(move || -> Result<()> {
            ...
            loop {
                let msg: Option<CoreMessage> =
                    devforge_rpc::stdio::read_msg(&mut reader)?;
