...
pub fn get_latest_release() -> Result<ReleaseInfo> {
    let url = match meta::RELEASE {
        ...
        meta::ReleaseType::Nightly => {
            "https://api.github.com/repos/bimacoding/DevForge/releases/tags/nightly"
        }
        _ => "https://api.github.com/repos/bimacoding/DevForge/releases/latest",
    };

    let resp = devforge_proxy::get_url(url, Some("DevForge"))?;
...
pub fn download_release(release: &ReleaseInfo) -> Result<PathBuf> {
    ...
    let name = match std::env::consts::OS {
        "macos" => "DevForge-macos.dmg",
        "linux" => match std::env::consts::ARCH {
            "aarch64" => "devforge-linux-arm64.tar.gz",
            "x86_64" => "devforge-linux-amd64.tar.gz",
            _ => return Err(anyhow!("arch not supported")),
        },
        #[cfg(feature = "portable")]
        "windows" => "DevForge-windows-portable.zip",
        #[cfg(not(feature = "portable"))]
        "windows" => "DevForge-windows.msi",
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::remove_dir_all(dest.join("DevForge.app"))?;
    fs_extra::copy_items(
        &[info.mount_point.join("DevForge.app")],
        dest,
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::copy(parent.join("DevForge").join("devforge"), process_path)?;
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::rename(process_path, dst_parent.join("devforge.exe.bak"))?;
    std::fs::copy(parent.join("devforge.exe"), process_path)?;
...
pub fn restart(path: &Path) -> Result<()> {
    ...
    let devforge_exe = std::env::current_exe()
        .map_err(|err| anyhow!("can't get path to exe").context(err))?;
    let devforge_exe = devforge_exe
        .to_str()
        .ok_or_else(|| anyhow!("can't convert exe path to str"))?;

    std::process::Command::new("cmd")
        .raw_arg(format!(
            r#"/C taskkill /PID {process_id} & msiexec /i "{path}" /qb & start "" "{devforge_exe}""#,
        ))
        .creation_flags(DETACHED_PROCESS)
        .spawn()?;
    Ok(())
