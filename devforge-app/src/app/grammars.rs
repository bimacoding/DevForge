...
fn get_github_api(url: &str) -> Result<String> {
    let user_agent = format!("Lapce/{}", devforge_core::meta::VERSION);
    let resp = devforge_proxy::get_url(url, Some(user_agent.as_str()))?;
...
pub fn find_grammar_release() -> Result<ReleaseInfo> {
    let releases: Vec<ReleaseInfo> = serde_json::from_str(&get_github_api(
        "https://api.github.com/repos/bimacoding/tree-sitter-grammars/releases?per_page=100",
    ).context("Failed to retrieve releases for tree-sitter-grammars")?)?;

    use devforge_core::meta::{RELEASE, ReleaseType, VERSION};
