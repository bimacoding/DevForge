...
impl EditorData {
    ...
    fn run_edit_command(&self, cmd: &EditCommand) -> CommandExecuted {
        ...
        let yank_data =
            if let devforge_core::cursor::CursorMode::Visual { .. } = &cursor.mode {
    ...
    fn run_move_command(
        &self,
        movement: &devforge_core::movement::Movement,
        count: Option<usize>,
        mods: Modifiers,
    ) -> CommandExecuted {
    ...
    fn inline_find(&self, direction: InlineFindDirection, c: &str) {
        ...
        if let Some(new_index) = match direction {
            ...
            self.run_move_command(
                &devforge_core::movement::Movement::Offset(
    ...
    fn on_screen_find(&self, pattern: &str) -> Vec<SelRegion> {
        ...
        if let Some((_, offset, _)) = items.first().copied() {
            self.run_move_command(
                &devforge_core::movement::Movement::Offset(offset),
                None,
                Modifiers::empty(),
            );
        }
    ...
    fn apply_completion_item(&self, item: &CompletionItem) -> anyhow::Result<()> {
        ...
        let additional_edit: Vec<_> = item
            ...
            .flatten()
            .map(|edit| {
                let selection = devforge_core::selection::Selection::region(
                    buffer.offset_of_position(&edit.range.start),
                    buffer.offset_of_position(&edit.range.end),
                );
                (selection, edit.new_text.as_str())
            })
            .collect::<Vec<(devforge_core::selection::Selection, &str)>>();
    ...
    pub fn completion_apply_snippet(
        ...
        let mut selection = devforge_core::selection::Selection::new();
        let (_tab, (start, end)) = &snippet_tabs[0];
        let region = devforge_core::selection::SelRegion::new(*start, *end, None);
        selection.add_region(region);
        cursor.set_insert(selection);
    ...
    fn search_whole_word_forward(&self, mods: Modifiers) {
        ...
        if let Some((start, _end)) = next {
            self.run_move_command(
                &devforge_core::movement::Movement::Offset(start),
                None,
                mods,
            );
        }
    ...
    pub fn word_at_cursor(&self) -> String {
        ...
        let region = self.cursor().with_untracked(|c| match &c.mode {
            devforge_core::cursor::CursorMode::Normal(offset) => {
                devforge_core::selection::SelRegion::caret(*offset)
            }
            devforge_core::cursor::CursorMode::Visual {
                start,
                end,
                mode: _,
            ...
            devforge_core::cursor::CursorMode::Insert(selection) => {
