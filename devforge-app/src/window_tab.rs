        ...
        if !change_plugins.is_empty() {
            ...
            if config.core.auto_reload_plugin {
                let mut plugin_metas: HashMap<
                    String,
                    devforge_rpc::plugin::VoltMetadata,
                > = self
                    .plugin
                    .installed
                    .get_untracked()
        ...
        match cmd {
            ...
            InstallToPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        ...
                        arguments: vec![String::from("-e"), format!(r#"do shell script "ln -sf '{}' /usr/local/bin/devforge" with administrator privileges"#, std::env::args().next().unwrap())],
                    }
                )
            }
            #[cfg(target_os = "macos")]
            UninstallFromPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        program: String::from("osascript"),
                        arguments: vec![String::from("-e"), String::from(r#"do shell script "rm /usr/local/bin/devforge" with administrator privileges"#)],
                    }
                )
            }
            JumpLocationForward => {
    ...
    fn handle_core_notification(&self, rpc: &CoreNotification) {
        ...
        match rpc {
            ...
            CoreNotification::Log {
                ...
                use devforge_rpc::core::LogLevel;
