...
impl PluginData {
    ...
    pub fn volt_installed(&self, volt: &VoltMetadata, icon: &Option<Vec<u8>>) {
        ...
        if !is_latest {
            ...
            std::thread::spawn(move || {
                let info: Option<VoltInfo> = devforge_proxy::get_url(url, None)
                    .ok()
    ...
    fn load_icon(volt: &VoltInfo) -> Result<VoltIcon> {
        ...
        let content = match cache_content {
            ...
            None => {
                let resp = devforge_proxy::get_url(&url, None)?;
    ...
    fn query_volts(query: &str, offset: usize) -> Result<VoltsInfo> {
        ...
        let plugins: VoltsInfo = devforge_proxy::get_url(url, None)?.json()?;
        Ok(plugins)
    }
